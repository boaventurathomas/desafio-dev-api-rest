...
export class TransacaoService {
  ...
  async consultaValorSaqueDia(agencia: string, conta: string, data: string): Promise<number> {
    ...
    if (transacoesDia.totalDia === null) {
      return 0
    } else {
      return transacoesDia.totalDia * -1;
    }
  }
  ...
  async saque(saqueDto: CreateTransacaoDto) {
    ...
    try {
      const transacaoRepository: Repository<Transacao> = connection.getRepository(Transacao);
      const contaRepository: Repository<Conta> = connection.getRepository(Conta);
      const valorSaque = +saqueDto.valor;

      const contaCadastrada = await this.buscaConta(saqueDto.conta, contaRepository);
      ...
      if (!this.validaSaldoContaTransacao(contaCadastrada, valorSaque)) {
        return badRequest('Saldo insuficiente')
      }

      const valorTotalResgatadoDia = await this.consultaValorSaqueDia(contaCadastrada.agencia, contaCadastrada.conta, getDateNowToDB());

      if (!this.validaLimite(contaCadastrada, valorSaque, valorTotalResgatadoDia)) {
      ...
      const novoSaldo = (+contaCadastrada.saldo) - valorSaque;
